  ...
  AudioController::getSharedInstance()->performOnAudioThread(
    [this, &res, codec](){
      ...
      if (webrtcChannelId_ < 0)
        res = RESULT_ERR;
     //liupenghui, added for micphone.
      voeCodec_->SetSendCodec(webrtcChannelId_, WebrtcAudioChannel::instFromCodec(codec));
      voeCodec_->SetRecPayloadType(webrtcChannelId_, WebrtcAudioChannel::instFromCodec(Codec::G722));
      voeCodec_->SetFECStatus(webrtcChannelId_,1);
    });
  if (RESULT_FAIL(res))

...
InterestControl::InterestControl(const boost::shared_ptr<DrdEstimator> &drdEstimator,
                                 const boost::shared_ptr<statistics::StatisticsStorage> &storage,MediaStreamParams::MediaStreamType type,
    ...
    : initialized_(false),
      ...
      drdEstimator_(drdEstimator), sstorage_(storage), strategy_(strategy),type_(type),
      targetRate_(0.)
{
...
void InterestControl::continueadd(int delta)
{
    lowerLimit_ += delta;
    limit_ += delta;
    upperLimit_ += delta;
    changeLimitTo(limit_);
    setLimits();
    LogDebugC << "continueadd limit_: " << limit_ << std::endl;
...
void InterestControl::setLimits()
{
        ...
        upperLimit_ != newUpper)
    {
        if (!limitSet_ || newLower > lowerLimit_)
			...
			//liupenghui,  for audio sample fetching... 	
        if(type_ == MediaStreamParams::MediaStreamType::MediaStreamTypeAudio)
        {
           if (newUpper > upperLimit_)  
              upperLimit_ = newUpper;                            
        }
        else   
            upperLimit_ = newUpper;

                         ...
                         boost::shared_ptr<PipelineControlStateMachine::Struct> ctrl)
    {
        if (metadata)
        {
            ...
            unsigned int pipelineInitial =
                ctrl->interestControl_->getCurrentStrategy()->calculateDemand(metadata->getRate(),
                                                                              initialDrd, initialDrd * 0.05);
               std::cout<<"metadata->getRate():"<<(double)metadata->getRate()<<" initialDrd:"<<(int)initialDrd<<" bundleNo:"<<(int)bundleNo<<" pipelineInitial:"<<(int)pipelineInitial<<std::endl; 
			   //liupenghui,  for audio sample fetching, at least 1 for one sample,100 will be better for pipeliner limit...	   

            ctrl->sampleEstimator_->bootstrapSegmentNumber(1, SampleClass::Delta, SegmentClass::Data);
            ctrl->interestControl_->initialize(metadata->getRate(), pipelineInitial<100?100:pipelineInitial);
            ...
            //startOffSeqNums_.first = bundleNo + pipelineInitial<100?100:pipelineInitial; will wait, no need to wait so long.
                       ...
                       public ReceivedMetadataProcessing<MetadataClass>
{
  ...
  protected:
    ...
    std::string onStart(const boost::shared_ptr<const PipelineControlEvent> &) override
    {
        //askMetadata();
...
void Adjusting::enter()
{
    ...
    std::string Adjusting::onStart(const boost::shared_ptr<const PipelineControlEvent> &) 
    {
...
//******************************************************************************
    std::string Fetching::onStart(const boost::shared_ptr<const PipelineControlEvent> &) 
    {
       // ctrl_->pipeliner_->fillUpPipeline(ctrl_->threadPrefix_);
        return str();
    } 

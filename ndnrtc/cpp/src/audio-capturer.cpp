...
nRtp_(0), nRtcp_(0)
{
    ...
    std::cout<<"deviceIdx "<<deviceIdx<<"  nDevices"<<nDevices<<std::endl;
	...
	//liupenghui,  for audio sample fetching... 	
     ...
     audioDeviceModule_->MaxMicrophoneVolume(&maxvolume);  
     audioDeviceModule_->SetMicrophoneVolume(maxvolume);  
     bool enabled =0;
     audioDeviceModule_->MicrophoneMute(&enabled);  
     bool mute = 0;
     audioDeviceModule_->SetMicrophoneMute(mute);  
...
void AudioCapturerImpl::startCapture()
{
    ...
    if (RESULT_GOOD(res))
    {
        ...
        std::cout<<"startCapture started"<<std::endl;
    }
    else
    {
        stringstream ss;
...
void AudioCapturerImpl::stopCapture()
{
    ...
    std::cout<<"startCapture stopped"<<std::endl;
}

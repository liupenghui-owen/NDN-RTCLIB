...
RemoteStreamImpl::RemoteStreamImpl(asio::io_service &io,
                                   ...
                                   const std::string &streamPrefix, MediaStreamParams::MediaStreamType type)
    : type_(type)
    , io_(io)
    , face_(face)
    , keyChain_(keyChain)
...
void RemoteStreamImpl::construct()
{
    ...
    //liupenghui timeout must be equal at least to interest timeout
    ...
    interestControl_ = make_shared<InterestControl>(drdEstimator_, sstorage_, type_);
    ...
    //liupenghui,bootstrap...
#if 1
    sampleEstimator_->bootstrapSegmentNumber(1, SampleClass::Delta, SegmentClass::Data);
    sampleEstimator_->bootstrapSegmentNumber(0, SampleClass::Delta, SegmentClass::Parity);
    sampleEstimator_->bootstrapSegmentNumber(0, SampleClass::Key, SegmentClass::Data);
    sampleEstimator_->bootstrapSegmentNumber(0, SampleClass::Key, SegmentClass::Parity);
#endif    
}
